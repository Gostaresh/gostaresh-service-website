<template>
  <section class="container mx-auto px-4 py-8" dir="rtl">
    <div class="mb-5 flex items-center justify-between gap-3">
      ...
      <NuxtLink to="/admin/brands/create" class="inline-flex items-center gap-2 rounded-xl bg-sky-600 text-white px-3 py-2 text-sm">
...
async function fetchList() {
  pending.value = true
  try {
    const data: any = await apiGet('/brands', { params: { q: q.value || undefined, limit: limit.value, offset: offset.value } })
...
const columns = computed(() => [
  ...
  { title: 'فعال', key: 'isActive', render: (row: any) => (row.isActive ? 'بله' : 'خیر') },
  {
    title: 'عملیات',
    key: 'actions',
    render(row: any) {
