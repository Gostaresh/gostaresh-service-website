<template>
  <section class="container mx-auto px-4 py-8" dir="rtl">
    <div class="mb-5 flex items-center justify-between gap-3">
      ...
      <NuxtLink to="/admin/roles/create" class="inline-flex items-center gap-2 rounded-xl bg-sky-600 text-white px-3 py-2 text-sm">
...
async function fetchList() { pending.value = true; try { items.value = (await apiGet('/roles')) as any[] } finally { pending.value = false } }
onMounted(fetchList)

function onEdit(row: any) { navigateTo(`/admin/roles/${row.id}`) }
async function onDelete(row: any) { if (!confirm('حذف نقش؟')) return; try { await apiDelete(`/roles/${row.id}`); msg.success('حذف شد'); fetchList() } catch (e: any) { msg.error(e?.data?.message || 'خطا در حذف') } }
