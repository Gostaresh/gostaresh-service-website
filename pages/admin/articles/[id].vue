<template>
  <section class="container mx-auto px-4 py-8" dir="rtl">
    <div class="mb-5 flex items-center justify-between gap-3">
      <h1 class="text-xl font-bold">ویرایش مقاله</h1>
...
onMounted(async () => { await Promise.all([load(), fetchTypes()]) })

async function load() { try { Object.assign(form.value, await apiGet(`/articles/${id}`) || {}) } catch { msg.error('خواندن مقاله ناموفق بود') } }
async function fetchTypes() { try { types.value = await apiGet('articles/types') as any[] } catch {} }
...
async function ensureSlug() { if (!form.value.title || form.value.slug) return; try { const { slug } = await $apiFetch<{ slug: string }>(`/utils/slug/preview`, { method: 'POST', body: { model: 'article', source: form.value.title, excludeId: id } }); if (slug) form.value.slug = slug } catch {} }

async function onSubmit() { try { await formRef.value?.validate() } catch { return } ; saving.value = true; try { await apiPut(`/articles/${id}`, { title: form.value.title, articleTypeID: form.value.articleTypeID || undefined, shortContent: form.value.shortContent || undefined, longContent: form.value.longContent || undefined, slug: form.value.slug || undefined, isActive: form.value.isActive }); msg.success('ذخیره شد') } catch (e: any) { msg.error(e?.data?.message || 'خطا در ذخیره') } finally { saving.value = false } }
async function onDelete() { if (!confirm('حذف مقاله؟')) return; deleting.value = true; try { await apiDelete(`/articles/${id}`); navigateTo('/admin/articles') } catch (e: any) { msg.error(e?.data?.message || 'خطا در حذف') } finally { deleting.value = false } }
</script>
