<template>
  <section class="container mx-auto px-4 py-8" dir="rtl">
    <div class="mb-5 flex items-center justify-between gap-3">
      <h1 class="text-xl font-bold">افزودن نوع تنظیمات</h1>
...
async function onSubmit() { try { await formRef.value?.validate() } catch { return } ; submitting.value = true; try { await apiPost('/website-setting-kinds', { name: form.value.name }); msg.success('ثبت شد'); navigateTo('/admin/website-setting-kinds') } catch (e: any) { msg.error(e?.data?.message || 'خطا در ثبت') } finally { submitting.value = false } }
</script>
